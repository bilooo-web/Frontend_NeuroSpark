...
const StoryBook = () => {
  ...
  const [pageSummaries, setPageSummaries] = useState([]); // one entry per page
  const [isTTSSpeaking, setIsTTSSpeaking] = useState(false); // true while giraffe TTS is reading
  ...
  const goToNextPage = () => {
    ...
    if (isLastPageRef.current) {
      // Save FULL stats to localStorage so the intro page can display them
      try {
        // Build the complete pageSummaries including the just-built last page
